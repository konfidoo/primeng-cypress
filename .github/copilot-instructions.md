...
│   ├── pTabs.ts           # Example: pTabs helper implementation
│   ├── pToggleSwitch.ts   # Example: pToggleSwitch helper implementation (added)
│   ├── types.ts           # Type definitions
│   └── index.ts           # Public API exports
├── cypress/               # Example Cypress tests and support files
│   ├── pButton.cy.ts
│   ├── pTabs.cy.ts
│   └── pToggleSwitch.cy.ts  # Example tests for the toggle helper
├── docs/                  # Per-command documentation
│   ├── pButton.md
│   ├── pTabs.md
│   └── pToggleSwitch.md   # Documentation for the toggle helper
...
8. **Verify changes**:
   ...
   npx tsc --noEmit
   npm run build:lib
   npx cypress run --component --headless --spec "cypress/<name>.cy.ts"
   ```
...
- Include positive and edge case tests, for example:
  - Combined options (click + expectChecked + expectClasses)
  - Disabled state validation (verify helper recognizes `disabled` and avoids clicking)
